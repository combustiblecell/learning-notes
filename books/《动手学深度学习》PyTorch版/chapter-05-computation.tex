...
% BOOKFIG-BEGIN blocks.pdf
\bookfig{blocks.pdf}{5.1.1}{多个层被组合成块，形成更大的模型}{层：基本运算；块：层的组合；模型：块的堆叠}
% BOOKFIG-END blocks.pdf
...
并非所有块都是纯顺序复合：残差块在前向里做 $\bm{Y}\leftarrow F(\bm{X})+\bm{X}$，稠密块做通道维连结 $\bm{X}\leftarrow[\bm{X},F(\bm{X})]$。
二者都必须写在自定义前向中；$\operatorname{Seq}$ 不会自动相加或拼接。相加要求形状相同，连结只增加通道个数。
...
% BOOKFIG-BEGIN copyto.pdf
\bookfig{copyto.pdf}{5.6.1}{复制数据以在同一设备上执行操作}{CPU/GPU：设备；箭头：张量拷贝到同一设备再运算}
% BOOKFIG-END copyto.pdf
