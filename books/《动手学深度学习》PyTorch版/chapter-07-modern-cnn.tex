...
记一张图为 $\mathsf{X}$、标签为 $y$。传统视觉流水线先用固定特征映射 $\phi$ 再接分类器 $g$，
\[
\hat{y}
=
g\bigl(\phi(\mathsf{X})\bigr);
\]
$\phi(\mathsf{X})$ 是手工设计的特征向量（各分量是 SIFT、HOG 等描述子），$g(\phi(\mathsf{X}))$ 是类别打分或标签。
卷积神经网络改为端到端映射
\[
f_{\bm{\theta}}
=
...
f_{\bm{\theta}}(\mathsf{X}),
\]
输入 $\mathsf{X}$ 是像素张量，输出 $\hat{\bm{o}}$ 是各类 logit；每一层 $f_\ell$ 的参数都由数据学习，而不再冻结 $\phi$。

\subsection{学习表征}

2012 年前主导的是 SIFT、SURF、HOG 与视觉词袋等固定 $\phi$。
另一路线主张特征本身由多层共同学习：最底层检测边缘与纹理，更高层组合成物体部件，直至类别可分。
第 $\ell$ 层输出 $\mathsf{H}^{(\ell)}$ 仍是特征图：每个空间--通道格点上的数是该层局部模式分数。

\subsubsection{缺少的成分：数据}

深度 $f_{\bm{\theta}}$ 参数多，经验风险
\[
\hat{R}_n(f_{\bm{\theta}})
=
\frac{1}{n}\sum_{i=1}^{n}\ell\bigl(f_{\bm{\theta}}(\mathsf{X}_i),y_i\bigr)
\]
需要足够大的 $n$ 才优于凸方法。左边是标量平均损失；$\ell$ 是单样本分类损失，$n$ 是有标签样本个数。
早期公开数据往往只有数百至数千张低分辨率图。
ImageNet 提供约 $n=10^{6}$ 张图、$q=10^{3}$ 类，使过参数化卷积网的 $\hat{R}_n$ 可被数据约束。
$10^{6}$ 是样本个数，$10^{3}$ 是类别个数，都不是像素强度。

\subsubsection{缺少的成分：硬件}

每轮训练要多次穿过线性代数层。GPU 原本优化 $4\times 4$ 矩阵--向量积，与卷积中的局部乘加同类。
相对 CPU，GPU 以高吞吐并行乘加降低器件上的计算时间 $T_{\mathrm{device}}$（单位是秒），使更深 $f$ 可训练。

\subsection{AlexNet}

AlexNet 为八层可学习映射：五层卷积、两层全连接隐层、一层全连接输出，并在 ImageNet 上显著超过手工 $\phi$。
与 LeNet 同构但更深更宽，且用 $\operatorname{ReLU}$ 替代 $\sigma$。

\subsubsection{模型设计}

输入按 $n_h=n_w=224$ 计，这两个整数是输入图像的空间高宽。
填充--步幅公式（第 6 章）给出输出高宽
\[
n_h'
=
\biggl\lfloor
...
\biggr\rfloor,
\]
$k_h$ 是核高，$p_h$ 是沿高方向的总填充格点数，$s_h$ 是步幅；算出的 $n_h'$ 是特征图行数，不是响应强度。
第一层核 $11\times 11$、步幅 $4$、每边填充 $1$（故 $p_h=p_w=2$），匹配更大目标：
\[
\biggl\lfloor
\frac{224-11+2+4}{4}
\biggr\rfloor
...
54.
\]
该 $54$ 是第一层特征图的空间边长。随后核为 $5\times 5$ 与 $3\times 3$。
在第一、第二、第五卷积后接 $3\times 3$、步幅 $2$ 的最大汇聚：输出每个条目是窗口内最强响应，高宽仍由上式给出。
通道数约为 LeNet 的十倍。书中精简实现（单 GPU、单通道输入、输出 $10$ 类）的形状链为
\begin{align*}
(1,1,224,224)
&\mapsto
(1,96,54,54)
...
(1,10).
\end{align*}
括号内整数依次是批量、通道（或向量长度）、高、宽这些轴长度，不是激活值。
展平后 $256\cdot 5\cdot 5=6400$ 是送入全连接的特征维数；末两层隐宽各为 $4096$，每个分量是该隐单元的激活；
最后 $10$ 维（演示）或 $1000$ 维（原论文）是类别 logit。
...
\]
输入 $u$ 是仿射打分；$\operatorname{ReLU}(u)$ 在 $u>0$ 时等于该打分，否则为 $0$，表示该单元未激活。
$\sigma(u)\in(0,1)$ 是挤压后的活性。饱和区
\[
\sigma'(u)=\sigma(u)\bigl(1-\sigma(u)\bigr)\approx 0,
\]
该导数是损失经该点回传的局部斜率，接近 $0$ 表示参数几乎不被更新。
$\operatorname{ReLU}$ 在正半轴导数恒为 $1$，对初始化更不敏感，且无需指数。

\subsubsection{容量控制和预处理}

全连接层用暂退法（dropout，第 4.6 节）约束复杂度。训练时
\[
h'_j=\frac{M_j}{1-p}h_j,
\qquad
M_j\sim\mathrm{Bernoulli}(1-p),
\]
$h_j$ 是隐激活，$p$ 是丢弃概率（书中全连接后取 $p=0.5$），$M_j\in\{0,1\}$ 表示丢弃或保留；
除以 $1-p$ 使 $\mathbb{E}[h'_j]=h_j$。测试时 $h'=h$。
训练输入再经翻转、裁切、变色等随机变换 $T$，在增广分布上最小化
\[
\mathbb{E}_{T}\bigl[\ell\bigl(f_{\bm{\theta}}(T(\mathsf{X})),y\bigr)\bigr].
\]
$\ell$ 是单样本损失（标量）；期望是对随机增广 $T$ 求平均。
该标量仍表示增广后的预测与标签有多不符，与未经增广时的损失同量纲。

\subsection{读取数据集}

原论文在 ImageNet 上训练。为缩短演示，把 Fashion-MNIST 的 $28\times 28$ 升到 $224\times 224$，
以便套用同一套核与步幅，尽管这并非该数据上的最优预处理。
重采样改变的是空间高宽这两个整数；每个格点上的数仍表示亮度或颜色。

\subsection{训练AlexNet}

分类头输出 $\hat{\bm{o}}\in\mathbb{R}^{q}$，训练最小化交叉熵
\[
L
=
-\frac{1}{m}\sum_{i=1}^{m}\log
\frac{\exp(\hat{o}_{i,y_i})}{\sum_{k=1}^{q}\exp(\hat{o}_{i,k})}.
\]
$m$ 是批量大小，$q$ 是类别数；分式是 softmax 后真实类的预测概率，$L$ 是标量平均负对数似然。
参数更新
\[
\bm{\theta}
\leftarrow
\bm{\theta}-\eta\nabla_{\bm{\theta}}L.
\]
$\eta$ 是步长（每单位梯度把参数挪多远）；$\nabla_{\bm{\theta}}L$ 的每个分量是该参数增大时损失大约增加多少。
与 LeNet 相比，深度、宽度与分辨率都更大，书中取更小的 $\eta=0.01$，否则一步容易跨过合适区域。

\subsection{小结}

AlexNet 把 LeNet 式的 $\operatorname{Conv}\circ\operatorname{ReLU}\circ\operatorname{Pool}$ 加深加密，
并用暂退与增广控制假设类 $\mathcal{F}$ 的有效容量。
卷积条目是局部模式分数，汇聚条目是最强响应，最后全连接给出类别 logit。
形状由填充--步幅公式决定，是轴长度；激活与损失才是响应强度与误差。
...
简化模型或直接在 $28\times 28$ 上工作，等于改 $n_h,n_w,c,k,s$，须重新代入形状公式。
