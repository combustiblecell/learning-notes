...
% BOOKFIG-BEGIN mlp.pdf
\bookfig{mlp.pdf}{4.1.1}{一个单隐藏层的多层感知机，具有5个隐藏单元}{$\bm{x}$：输入；隐层：$h$；$\bm{o}$：输出 logits}
% BOOKFIG-END mlp.pdf
...
此时一般不能再合并为单次仿射。
非线性来自 $\sigma$，不是来自把权重写成 $e^{w}$。
对 $\sigma=\operatorname{ReLU}$，若某隐单元 $z_j<0$ 则 $h_j=0$，该路被关掉；
输入从 $\bm{x}$ 换成 $2\bm{x}$ 时，被截断的路不必按比例恢复，故一般 $f(2\bm{x})\ne 2f(\bm{x})$。
...
% BOOKFIG-BEGIN capacity-vs-error.pdf
\bookfig{capacity-vs-error.pdf}{4.4.1}{模型复杂度对欠拟合和过拟合的影响}{横轴：模型容量；纵轴：误差；训练/泛化两条曲线}
% BOOKFIG-END capacity-vs-error.pdf
...
% BOOKFIG-BEGIN dropout2.pdf
\bookfig{dropout2.pdf}{4.6.1}{dropout前后的多层感知机}{实心：保留单元；虚线/空心：按概率丢弃的单元}
% BOOKFIG-END dropout2.pdf
...
% BOOKFIG-BEGIN forward.pdf
\bookfig{forward.pdf}{4.7.1}{前向传播的计算图}{$x$：输入；$y$：输出；$L$：损失；箭头：前向计算图}
% BOOKFIG-END forward.pdf
...
% BOOKFIG-BEGIN cat-dog-train.pdf
\bookfig{cat-dog-train.pdf}{4.9.1}{区分猫和狗的训练数据}{见图中标注；各块含义见题注。}
% BOOKFIG-END cat-dog-train.pdf

% BOOKFIG-BEGIN cat-dog-test.pdf
\bookfig{cat-dog-test.pdf}{4.9.2}{区分猫和狗的测试数据}{见图中标注；各块含义见题注。}
% BOOKFIG-END cat-dog-test.pdf

% BOOKFIG-BEGIN popvssoda.png
\bookfig{popvssoda.png}{4.9.3}{美国软饮名称的概念偏移}{见图中标注；各块含义见题注。}
% BOOKFIG-END popvssoda.png
