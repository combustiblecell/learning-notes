...
\end{align*}
$\bm{R}_t$ 靠近 $0$ 时清空历史，$\bm{Z}_t$ 靠近 $1$ 时倾向原样保留 $\bm{H}_{t-1}$。
...
\]
$\bm{R}_t=\bm{1}$ 时退化为普通候选隐状态；$\bm{R}_t=\bm{0}$ 时候选只看 $\bm{X}_t$。
...
以及输出层 $(\bm{W}_{hq},\bm{b}_q)$。隐藏宽度仍由 $h$ 给出。

\subsubsection{定义模型}

$\bm{H}_0=\bm{0}$。逐步计算 $(\bm{R}_t,\bm{Z}_t,\tilde{\bm{H}}_t,\bm{H}_t,\bm{O}_t)$，
参数在所有 $t$ 上共享。映射与定义式逐项相同。

\subsubsection{训练与预测}

损失、裁剪、困惑度与前缀续写均沿用循环神经网络从零实现中的流程。
训练后比较训练集困惑度以及给定前缀时的续写序列。
...
\]
用 $\bm{Z}_t$ 保持长程信息、用 $\bm{R}_t$ 控制短程候选；两门全开或全关时分别覆盖拷贝与普通 RNN。
...
\]
只有 $\bm{H}_t$ 进入 $\bm{O}_t^{\text{out}}=\bm{H}_t\bm{W}_{hq}+\bm{b}_q$；
$\bm{C}_t$ 是内部状态。
...
初始化 $(\bm{H}_0,\bm{C}_0)=(\bm{0},\bm{0})$。逐步计算
...
\]
其中 $P(h_1 \mid h_0)=P(h_1)$。边际化全部 $h$ 时，前向量
\[
\pi_{t+1}(h_{t+1})
=
...
\]
最常见的取法是 $q$ 只保留 $\bm{h}_T$，或取多层最终隐状态的拼接。
...
$P(y_1,\ldots,y_{T'}\mid\bm{c})$。逐步取最大是贪心；枚举全部序列是穷举；
束搜索在二者之间保留 $k$ 条候选。
...
束宽为 $k$ 时，每步只保留条件概率最高的 $k$ 条前缀。例如 $k=2$ 时，
若第一步留下 $A,C$，则第二步比较
\begin{align*}
