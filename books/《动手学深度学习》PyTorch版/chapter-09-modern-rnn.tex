...
% BOOKFIG-BEGIN gru-1.pdf
\bookfig{gru-1.pdf}{9.1.1}{在门控循环单元模型中计算重置门和更新门}{$R_t$：重置门；$Z_t$：更新门；$\bm{h}_{t-1}$：上一隐状态}
% BOOKFIG-END gru-1.pdf

% BOOKFIG-BEGIN gru-2.pdf
\bookfig{gru-2.pdf}{9.1.2}{在门控循环单元模型中计算候选隐状态}{见图中标注；各块含义见题注。}
% BOOKFIG-END gru-2.pdf

% BOOKFIG-BEGIN gru-3.pdf
\bookfig{gru-3.pdf}{9.1.3}{计算门控循环单元模型中的隐状态}{见图中标注；各块含义见题注。}
% BOOKFIG-END gru-3.pdf
...
% BOOKFIG-BEGIN lstm-0.pdf
\bookfig{lstm-0.pdf}{9.2.1}{长短期记忆模型中的输入门、遗忘门和输出门}{$I_t,F_t,O_t$：输入/遗忘/输出门；$\tilde{C}_t$：候选记忆}
% BOOKFIG-END lstm-0.pdf

% BOOKFIG-BEGIN lstm-1.pdf
\bookfig{lstm-1.pdf}{9.2.2}{长短期记忆模型中的候选记忆元}{见图中标注；各块含义见题注。}
% BOOKFIG-END lstm-1.pdf

% BOOKFIG-BEGIN lstm-2.pdf
\bookfig{lstm-2.pdf}{9.2.3}{在长短期记忆网络模型中计算记忆元}{见图中标注；各块含义见题注。}
% BOOKFIG-END lstm-2.pdf

% BOOKFIG-BEGIN lstm-3.pdf
\bookfig{lstm-3.pdf}{9.2.4}{在长短期记忆模型中计算隐状态}{见图中标注；各块含义见题注。}
% BOOKFIG-END lstm-3.pdf
...
% BOOKFIG-BEGIN deep-rnn.pdf
\bookfig{deep-rnn.pdf}{9.3.1}{深度循环神经网络结构}{见图中标注；各块含义见题注。}
% BOOKFIG-END deep-rnn.pdf
...
% BOOKFIG-BEGIN hmm.pdf
\bookfig{hmm.pdf}{9.4.1}{隐马尔可夫模型}{见图中标注；各块含义见题注。}
% BOOKFIG-END hmm.pdf

% BOOKFIG-BEGIN birnn.pdf
\bookfig{birnn.pdf}{9.4.2}{双向循环神经网络架构}{前向 $\overrightarrow{\bm{h}}$；后向 $\overleftarrow{\bm{h}}$；拼接为上下文}
% BOOKFIG-END birnn.pdf
...
% BOOKFIG-BEGIN encoder-decoder.pdf
\bookfig{encoder-decoder.pdf}{9.6.1}{编码器-解码器架构}{Encoder：源序列→状态；Decoder：状态→目标词}
% BOOKFIG-END encoder-decoder.pdf
...
% BOOKFIG-BEGIN seq2seq.pdf
\bookfig{seq2seq.pdf}{9.7.1}{使用循环神经网络编码器和循环神经网络解码器的序列到序列学习}{$\bm{h}$：编码器隐状态；$\bm{s}$：解码器状态；$y_t$：输出词}
% BOOKFIG-END seq2seq.pdf

% BOOKFIG-BEGIN seq2seq-details.pdf
\bookfig{seq2seq-details.pdf}{9.7.2}{循环神经网络编码器-解码器模型中的层}{见图中标注；各块含义见题注。}
% BOOKFIG-END seq2seq-details.pdf

% BOOKFIG-BEGIN seq2seq-predict.pdf
\bookfig{seq2seq-predict.pdf}{9.7.3}{使用循环神经网络编码器-解码器逐词元地预测输出序列。}{见图中标注；各块含义见题注。}
% BOOKFIG-END seq2seq-predict.pdf
...
% BOOKFIG-BEGIN s2s-prob1.pdf
\bookfig{s2s-prob1.pdf}{9.8.1}{在每个时间步，贪心搜索选择具有最高条件概率的词元}{见图中标注；各块含义见题注。}
% BOOKFIG-END s2s-prob1.pdf

% BOOKFIG-BEGIN s2s-prob2.pdf
\bookfig{s2s-prob2.pdf}{9.8.2}{在时间步2，选择具有第二高条件概率的词元“C”（而非最高条件概率的词元）}{见图中标注；各块含义见题注。}
% BOOKFIG-END s2s-prob2.pdf

% BOOKFIG-BEGIN beam-search.pdf
\bookfig{beam-search.pdf}{9.8.3}{束搜索过程（束宽：2，输出序列的最大长度：3）。候选输出序列是$A$、$C$、$AB$、$CE$、$ABD$和$CED$}{$k$：束宽；每步保留分数最高的 $k$ 条部分序列}
% BOOKFIG-END beam-search.pdf
