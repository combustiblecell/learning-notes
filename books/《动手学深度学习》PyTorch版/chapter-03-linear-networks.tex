...
\]
$p(x)$ 是密度，可以大于 $1$，不是概率；真正的概率是密度在区间上的积分。
$\mu$ 是分布的中心，$\sigma$ 是起伏尺度，与 $x$ 同量纲。
观测模型
\[
y=\bm{w}^{\mathsf T}\bm{x}+b+\epsilon
\]
...
\]
这里写成密度对 $y$ 的值：均值是点预测 $\bm{w}^{\mathsf T}\bm{x}+b$，残差越大则该密度越小。
独立样本的似然是
\[
P(\bm{y}\mid\bm{X})=\prod_{i=1}^{n}p(y^{(i)}\mid\bm{x}^{(i)}),
\]
把各样本密度相乘，表示“整批观测同时出现”的联合密度（连续情形下仍不是 $[0,1]$ 概率）。
负对数似然为
\begin{align*}
-\log P(\bm{y}\mid\bm{X})
&=
...
\end{align*}
$\sigma$ 固定时，后一项与均方误差 $L(\bm{w},b)$ 只差正常数，故极大似然与最小化 $L$ 等价：
...
核验 $\sum_{i}(x_{i}-b)^{2}$ 对 $b$ 的驻点：$b$ 应等于各 $x_i$ 的均值，即“典型水平”，不是某一个观测。
把 $b$ 并入 $\bm{X}$ 后正规方程是否仍给出同一组权重。
$\bm{X}^{\mathsf T}\bm{X}$ 奇异时解析解不存在，梯度法仍可沿“减小均方误差”的方向走。
